...
namespace Microsoft::CodePush::ReactNative
{
	...
	static winrt::Windows::Foundation::IAsyncOperation<hstring>
		ComputeFileSha256Async(const winrt::Windows::Storage::StorageFile& file)
	{
		using namespace winrt;
		using namespace Windows::Storage;
		...
		if (!file) co_return hstring{};           // ← null-safety
		...
		co_return CryptographicBuffer::EncodeToHexString(hash);  // lower-case hex
	}
	 ...
	 */
	fire_and_forget CodePushNativeModule::GetConfiguration(
		ReactPromise<IJsonValue> promise) noexcept
	{
		try
		{
			// base configuration (appVersion, deploymentKey, etc.)
			JsonObject configuration{ CodePushConfig::Current().GetConfiguration() };

			// When running the binary bundle, compute its hash once.
			if (isRunningBinaryVersion)
			{
				auto binaryBundle = co_await GetBinaryBundleAsync();   // may be null
				auto binaryHash = co_await ComputeFileSha256Async(binaryBundle);

				configuration.Insert(
					L"binaryHash",
					JsonValue::CreateStringValue(binaryHash.empty() ? L"" : binaryHash));

				// (Optional) mark explicitly that we’re on the original binary
				configuration.Insert(
					L"isRunningBinaryVersion",
